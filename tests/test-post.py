...
def main():
    ...
    t0 = timeit.timeit(lambda: make_sequential_requests(N), number=1)
    t1 = timeit.timeit(lambda: start_make_concurrent_requests(N), number=1)
    
    print(f'{N} Последовательных запросов закончены за {t0:.3} sec. Среднее время на запрос {t0/N:.3} sec.')
    print(f'{N} Параллельных запросов закончены за {t1:.3} sec. Среднее время на запрос {t1/N:.3} sec.')
